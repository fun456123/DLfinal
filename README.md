# AI 生成影像偵測：Branch A + Branch B 雙分支融合模型
...
- **Fusion Classifier：Feature-Level Fusion**  
  將 Branch A 與 Branch B 的特徵串接後，透過 MLP 輸出 real/fake 二元分類結果。
...
DLfinal/
  configs/
    cifake_branch_b.json
    cifake_fusion_a_b.json
    tiny_sdv5_branch_b.json
    tiny_sdv5_fusion_a_b.json
    eval_cifake.json
    eval_cifake_fusion_a_b.json
    eval_tiny_sdv5_fusion_a_b.json
  dataset/
    cifake/
    tiny-genimage/
  runs/
...
| `src/branch_b.py` | Branch B patch 鑑識分支，包含 FFT patch selector、patch encoder、attention pooling。 |
| `src/fusion.py` | Branch A + Branch B 融合模型。 |
...
Model
   ...
   |
   +--> Branch B: FFT patch selector + patch encoder + attention pooling
   |
   v
...
| `image_forensic` | 給 Branch B | RGB、可選 resize 到 `forensic_size`、轉 tensor |

Branch A 使用 ImageNet normalization，是因為 ResNet backbone 預設使用 ImageNet 預訓練權重。Branch B 則保留較直接的像素 tensor，避免 normalization 改變局部鑑識訊號。
...
```bash
python train.py --config configs/cifake_fusion_a_b.json --no-augment
```
...
Branch A 的優點是能理解影像整體內容，例如物體形狀、場景語意、構圖與高階視覺特徵。不過只看整張圖可能忽略 AI 生成影像常出現在局部紋理與頻率分布上的細節破綻，因此本專案加入 Branch B。

### Branch B：Patch-Level Forensic Branch

Branch B 實作於 `src/branch_b.py`，是本專案的局部鑑識分支。它不直接處理整張圖，而是先切 patch，再根據 FFT 頻率能量挑選最有代表性的 patch。
...
因為每張圖會選出多個 patch，Branch B 使用 attention pooling 將多個 patch feature 聚合為單一向量。
...
Branch B 不只單純串接 high 與 low features，還加入兩種交互特徵：
...
Branch B 也有獨立 classifier，可用於單分支實驗：
...
```text
Linear(branch_a_feature_dim + branch_b_feature_dim -> fusion_hidden_dim)
BatchNorm1d
ReLU
Dropout
...
等價於使用 CIFAKE 訓練 Branch A + Branch B 融合模型。
...
python train.py \
  ...
  --stride 8 \
  --top-k 4 \
  --output-dir runs/cifake_fusion_a_b
```
...
python train.py \
  ...
  --stride 8 \
  --top-k 4 \
  --output-dir runs/cifake_aug_fusion_a_b
```
...
python train.py \
  ...
  --stride 16 \
  --top-k 8 \
  --output-dir runs/tiny_sdv5_fusion_a_b
```
...
python train.py \
  ...
  --top-k 8 \
  --progress-every 10 \
  --output-dir runs/cifake_tiny_aug_fusion_a_b
```
...
```bash
python evaluate.py \
  --checkpoint runs/tiny_sdv5_fusion_a_b/best.pt \
  --dataset tiny-genimage \
  --split val \
  --generators sdv5 \
  --forensic-size 224 \
...
```bash
python evaluate.py \
  --checkpoint runs/cifake_tiny_aug_fusion_a_b/best.pt \
  --dataset both \
  --split test \
  --forensic-size 224 \
  --patch-size 32 \
...
python evaluate_distortions.py \
  ...
  --top-k 8 \
  --progress-every 10 \
  --output-json runs/cifake_tiny_aug_fusion_a_b/distortion_results.json \
  --output-csv runs/cifake_tiny_aug_fusion_a_b/distortion_results.csv
```
...
python train.py \
  ...
  --epochs 20 \
  --batch-size 64 \
  --output-dir runs/cifake_fusion_a_b_e20
```
...
`runs/cifake_fusion_a_b/history.json` 顯示，CIFAKE fusion 模型在 validation set 上表現穩定，最佳 validation AUROC 出現在 epoch 8：
...
- blur、resize、noise 對模型影響較大，代表 Branch B 所依賴的局部頻率與紋理訊號容易被這類干擾改變。
...
Branch A 擅長捕捉整張圖片的語意與高階視覺結構；Branch B 擅長捕捉 patch-level 的局部頻率與紋理異常。兩者融合後，模型不只依賴單一訊號。

### 2. FFT patch selection 提高鑑識分支效率

Branch B 並不是平均看所有 patch，而是根據高頻比例挑出最極端的 high-frequency 與 low-frequency patches。這讓模型更集中在可能包含生成痕跡的位置。
...
- Branch B 對 blur、resize、noise 等失真較敏感。
...
- Branch B 的 patch selection 是固定規則，不是可學習式 patch selection。
...
- 訓練 Branch A only、Branch B only 與 Fusion A+B 的 ablation study。
- 嘗試 ResNet50、ConvNeXt、EfficientNet 或 ViT 作為 Branch A backbone。
- 將 Branch B 的 patch selection 改為 learnable attention 或 differentiable top-k approximation。
...
python evaluate_distortions.py \
  ...
  --dataset cifake \
  --split test \
  --output-json runs/cifake_fusion_a_b/distortion_results.json \
  --output-csv runs/cifake_fusion_a_b/distortion_results.csv
```

## 總結

本專案實作了一個針對 AI 生成影像偵測的雙分支融合模型。Branch A 透過 ResNet 擷取全域語意特徵，Branch B 透過 FFT patch selection 與 CNN patch encoder 擷取局部鑑識特徵，最後以 MLP 進行 feature-level fusion。實驗結果顯示，模型在 CIFAKE 上能達到高 accuracy、F1 與 AUROC，並且具備完整的訓練、評估、config 管理與魯棒性測試流程。
